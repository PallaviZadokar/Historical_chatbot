...
# Ensure torch is correctly loaded
...
try:
    groq_api_key = st.secrets["SETTINGS"]["GROQ_API_KEY"]
    email_address = st.secrets["SETTINGS"]["EMAIL_ADDRESS"]
    email_password = st.secrets["SETTINGS"]["EMAIL_PASSWORD"]
except KeyError:
    st.error("Error: Missing secrets! Make sure you have set them in Streamlit's secrets manager.")
    st.stop()
